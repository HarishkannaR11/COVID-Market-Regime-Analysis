% Framework diagram for inclusion in main document
...
% Layer 8: Results
\node[result, minimum width=10cm, fill=orange!30] (results) at (0, -4) {\textbf{Paradox: Stability + Risk-Dominance}\\{\scriptsize +24\% persistence, NDS shift: 0→-2.01}};
...
\end{tikzpicture}
\caption{Neurocomputational Framework Architecture. The pipeline processes NIFTY Bank Index data through feature engineering, maps features to five neuroanatomically-inspired brain systems, employs XGBoost classifiers to determine binary activation states, constructs composite Neuro-Decision Score (NDS), and performs temporal/statistical analysis revealing the post-COVID market paradox of increased persistence with risk-dominance.}
\label{fig:framework}
\end{figure}
